    ...
    public class LocalGameManager : IGameManager
    {
        ...
        public void InitializeLevel()
        {
            ...
            if (owner.Room != null)
            {
                ...
                {
                    ConsoleLog.SLog("Room is closed - don't accept other players");
                }
            ...
            {
                // single player mode
                ConsoleLog.SLog("Initializing Single player game");
            }
        }
        ...
        internal IEnumerator MoveEnemies()
        {
            ...
            foreach (string key in enemies.Keys)
            {
                ...
                {
                    ConsoleLog.SLog("Enemy is null for key " + key + " ?!?!");
                }
            }
        ...
        {
            ConsoleLog.SLog("Initializing LocalGame!");
            ...
            foreach (PlayerInfo p in PlayerInfo.AllPlayers)
            {
                ...
                {
                    ConsoleLog.SLogWarning("PlayerController is null?? for " + p.Player.Name);
                }
        ...
        public void OnNewGameState(GameManager.GameStateData newState, List<ItemState> changes)
        {
            if (newState != null)
            {
                ...
                // the game state.
                ConsoleLog.SLog("Got new game state: " + newState);
            }

            if (changes != null && changes.Count > 0)
            {
                ConsoleLog.SLog("Got New changes from Remote! with " + changes.Count);

                // Only change the players and scores
                foreach (ItemState item in changes)
                {
                    ConsoleLog.SLog("Processing  ----> " + item.Name);

...
namespace NearbyDroids
{
    ...
    public class RemoteGameManager : IGameManager
    {
        ...
        public void InitializeLevel()
        {
            ...
            ConsoleLog.SLog("OnLevelLoaded - initialize the Game!");
        ...
        internal void InitGame(GameManager.GameStateData data)
        {
            if (data != null)
            {
                ConsoleLog.SLog("Initializing using " + data.CurrentPhase + ": " +
                    data.SerialNumber);

                // if we are moving to the next level of the game, reload!
                if (owner.Level != data.Level)
                {
                    ConsoleLog.SLog("Going from level " + owner.Level + " to " +
                        data.Level);
        ...
        {
            ConsoleLog.SLog("Processing " + changes.Count + " changes!");
            foreach (ItemState item in changes)
            {
                ConsoleLog.SLog("---> " + item.Name);
                ...
                else
                {
                    ...
                    ConsoleLog.SLog("Creating " + item.Name);

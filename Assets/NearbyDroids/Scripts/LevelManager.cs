...
namespace NearbyDroids
{
    ...
    public class LevelManager : MonoBehaviour
    {
        ...
        internal void BoardSetup()
        {
            if (boardHolder != null)
            {
                ConsoleLog.SLog("Boardholder exists, so skipping setup!");
                return;
            }
        ...
        public GameObject CreateItem(ItemState item)
        {
            ...
            {
                ConsoleLog.SLogWarning("Unknown tile set: " + item.TileSetName + " for " + item.Name);
                return null;
            }
        ...
        public void SetupScene(int level, List<ItemState> levelData)
        {
            ...
            foreach (ItemState item in levelData)
            {
                ...
                if (myObjects.ContainsKey(item.Name) && myObjects[item.Name] != null)
                {
                    ConsoleLog.SLog("Using existing object: " + item.Name);
                ...
                {
                    ConsoleLog.SLogWarning("Unknown tile set: " + 
                ...
                obj.name = item.Name;
                ConsoleLog.SLog("Creating " + obj.name + " at " + obj.transform.position);
